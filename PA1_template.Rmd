...
The mean number of steps per day is `r meanSteps` and the median number of steps per day is `r medianSteps`. [Aside: I wish I knew how to format these passed variables. It's not ideal here but it gets worse later!]
...
As can be seen in the time series plot, the 5-minute interval, that contains the maximum number of steps on average is interval # `r intervalMax`.
...
The total number of rows with NAs is `r numberNA`
...
The mean ignoring NAs was `r meanSteps` whereas with imputing NAs it is `r meanStepsImputed`.
The median ignoring NAs was `r medianSteps` whereas with imputing NAs it is `r medianStepsImputed`. [Aside: This is ugly. I can't figure out how to change the number format of these passed variables so that both sets display similarly - they are similar figures but for some reason one number displays normally and the other in scientific format. DANG... Sorry!).
